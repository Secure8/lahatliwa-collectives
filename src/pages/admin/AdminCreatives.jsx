      ...
      {loading ? <LoadingState label="Loading creatives" /> : creatives.length ? (
        <section className="overflow-hidden rounded-lg border border-white/[0.1] bg-zinc-900">
          <div className="grid gap-3 border-b border-white/[0.1] p-4 sm:grid-cols-[minmax(14rem,1fr)_auto] sm:items-center">
            <label data-search-shell className="flex h-10 items-center gap-2 rounded-md border border-white/[0.12] bg-zinc-950 px-3"><Search size={15} className="text-zinc-600" /><span className="sr-only">Search creative profiles</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, role, or slug" className="min-w-0 flex-1 border-0 bg-transparent text-sm text-white outline-none placeholder:text-zinc-600" /></label>
