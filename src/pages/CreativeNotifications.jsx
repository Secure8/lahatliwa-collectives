...
export default function CreativeNotifications() {
  ...
  return <div className="page-shell ll-notifications-page">
    <header className="ll-notifications-heading"><div><p className="ll-kicker"><Bell size={14}/> Private inbox</p><h1>Notifications</h1><p>New connection requests and inquiries sent directly to you appear here.</p></div>{items.some((item) => !item.read_at) && <IconLabelAction icon={<CheckCheck size={17}/>} label="Mark all read" onClick={() => markCreativeNotificationsRead(items.filter((item) => !item.read_at).map((item) => item.id)).then(() => setItems((current) => current.map((item) => ({ ...item, read_at: item.read_at || new Date().toISOString() }))))}/>}</header>
