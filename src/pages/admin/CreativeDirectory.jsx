...
export default function CreativeDirectory() {
  ...
  return <AdminLayout><div className="w-full max-w-6xl">
    <AdminPageHeader eyebrow="Collective" title="Creative Directory" description="Browse published creative profiles available to your current admin role." />
    ...
    <section className="grid gap-4 border-b border-white/[0.08] pb-6 sm:grid-cols-[minmax(0,1fr)_14rem]">
      <label className="grid gap-1.5 text-sm text-zinc-300"><span>Search directory</span><span className="flex items-center gap-2 border-b border-white/[0.12]"><Search size={15} className="text-zinc-600" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, role, bio, or skill" className="w-full bg-transparent px-0 py-2.5 text-white outline-none placeholder:text-zinc-700" /></span></label>
    ...
    {loading ? <DirectorySkeleton /> : error ? <div className="border-b border-red-300/15 py-8"><p className="text-sm text-red-200">{error}</p><button type="button" onClick={loadDirectory} className="mt-3 border-b border-red-200/30 pb-1 text-sm text-red-100">Retry loading directory</button></div> : visibleRows.length ? <div className="divide-y divide-white/[0.08]">{visibleRows.map((creative) => <article key={creative.id} className="grid gap-4 py-5 sm:grid-cols-[3.5rem_minmax(0,1fr)_auto] sm:items-center"><div className="grid h-14 w-14 place-items-center overflow-hidden rounded-full bg-white/[0.04] text-zinc-500">{creative.profile_image_url ? <img src={creative.profile_image_url} alt={`${creative.name} profile`} loading="lazy" decoding="async" className="h-full w-full object-cover" /> : creative.name?.slice(0, 1)}</div><div className="min-w-0"><h2 className="truncate font-medium text-white">{creative.name}</h2><p className="mt-1 text-sm text-zinc-400">{creative.role}</p>{creative.availability_status && <p className="mt-1 text-xs text-zinc-600">{creative.availability_status}</p>}{creative.short_bio && <p className="mt-2 line-clamp-2 max-w-3xl text-sm leading-6 text-zinc-500">{creative.short_bio}</p>}</div><AdminButton to={`/creatives/${creative.slug}`} variant="ghost" className="w-fit"><ExternalLink size={15} /> View Profile</AdminButton></article>)}</div> : <AdminEmptyState title={rows.length ? 'No creatives match these filters' : 'No published creatives yet'} message={rows.length ? 'Adjust the search or availability filter.' : 'Published creative profiles will appear here.'} />}
  </div></AdminLayout>;
}
