...
import { buildInquirySubmissionRequest, changeInquiryBranchSelection, emptyInquiryDraft, INQUIRY_DRAFT_KEY, INQUIRY_SELECTION_STEP, INQUIRY_SPECIALIST_STEP, inquiryCopy, mergeInquiryContext, resolveInquiryEntry, safeInquiryDraft, validateInquiryStep } from '../lib/serviceRequest';
import { allServiceCategories, serviceByKey } from '../lib/serviceCatalog.js';
...
export default function StartProject() {
  ...
  const queryContext = useMemo(() => {
    ...
    return {
      path,
      branch: navigationSelection.legacyBranch || (path === 'tourism' ? 'tech' : path === 'general' ? 'general' : serviceByKey(searchParams.get('service') || navigationSelection.service || projectContext?.service)?.legacyBranch || ''),
  ...
  useEffect(() => {
    const configured = (content.websiteServices || []).map((service) => ({
      ...service,
      key: service.key || service.serviceKey,
      legacyBranch: service.branchKey || '',
    })).filter((service) => service.key && service.name && service.status !== 'inactive' && service.inquiryAvailability !== false);
    const fallback = allServiceCategories();
    setServiceOptions(configured.length ? [...configured, ...fallback.filter((service) => service.legacyBranch === 'general')] : fallback);
  }, [content.websiteServices]);
  ...
  useEffect(() => {
    ...
    const finishWithFallback = () => {
      ...
      settled = true;
      setCreatives([]);
      setChoiceLoadError('Specialist choices could not be verified. You can still continue without choosing a specific creative.');
  ...
  useEffect(() => {
    ...
    const entry = resolveInquiryEntry(queryContext, null, creatives);
    let contextNotice = '';
    setDraft((current) => {
      const next = mergeInquiryContext(current, queryContext);
      if (next.serviceKey && !serviceOptions.some((service) => service.key === next.serviceKey && service.legacyBranch === next.branch)) {
        next.branch = '';
        next.serviceKey = '';
        next.branchDetails = {};
        next.serviceMode = '';
      ...
      if (queryContext.branch) {
        ...
        if (entry.status === 'invalid-branch') contextNotice = 'The requested service is unavailable. Choose one below.';
        ...
        const matched = serviceOptions.find((service) => service.key === requestedService) || serviceByKey(requestedService);
        if (matched) Object.assign(next, changeInquiryBranchSelection(next, matched.legacyBranch), { serviceKey: matched.key });
  ...
  }, [creatives, loadingChoices, queryContext, serviceOptions]);
  ...
  const selectableServices = useMemo(() => availableServices.filter((service) => service.legacyBranch !== 'general'), [availableServices]);
  const selectedService = availableServices.find((service) => service.key === draft.serviceKey && (!draft.branch || service.legacyBranch === draft.branch)) || null;
  const selectedCreative = creatives.find((creative) => creative.slug === draft.creativeSlug) || null;
  const copy = inquiryCopy(inquiryPath === 'tourism' ? 'tech' : 'general');
  ...
  function selectService(serviceKey) {
    ...
    if (!service || (draft.serviceKey === serviceKey && draft.branch === service.legacyBranch)) return;
    setDraft((current) => ({ ...changeInquiryBranchSelection(current, service.legacyBranch), serviceKey }));
  ...
  async function submit() {
    ...
    try {
      ...
      const confirmation = { reference: data.reference, service: selectedService?.name || 'General inquiry', creative: selectedCreative?.name || '', submittedAt: data.submittedAt || new Date().toISOString() };
      try {
  ...
  return <div className="public-inquiry-flow page-shell py-16 sm:py-20" style={{ '--project-accent': content.accentColor, '--project-secondary': content.secondaryTextColor }}>
    ...
    {step > INQUIRY_SELECTION_STEP && selectedService && <SelectionSummary service={selectedService.name} specialist={selectedCreative?.name || (step > INQUIRY_SPECIALIST_STEP ? copy.teamOption : '')} onChange={inquiryPath === 'service' ? changeSelection : changeInquiryPath} onChangeSpecialist={inquiryPath === 'service' && step > INQUIRY_SPECIALIST_STEP ? changeSpecialist : null} />}
    ...
    <div className="grid gap-9 pt-8 lg:grid-cols-[minmax(0,1fr)_19rem] lg:gap-12">
      <section aria-labelledby="inquiry-step-heading" className="min-w-0 py-7">
        ...
        {step === 0 && <ServiceStep draft={draft} availableServices={selectableServices} selectService={selectService} serviceCategoryRef={serviceCategoryRef} loading={loadingChoices} errors={errors} copy={copy} />}
        ...
        {step === 4 && <ReviewStep draft={draft} service={selectedService} creative={selectedCreative} copy={copy} />}
...
function SelectionSummary({ service, specialist = '', onChange, onChangeSpecialist }) {
  return <section aria-label="Inquiry selections" className="mt-8 flex flex-col gap-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div aria-live="polite" aria-atomic="true"><p className="text-[10px] uppercase tracking-[0.18em] text-orange-300">Selected service</p><p className="mt-1 text-sm font-medium text-white">{service}</p>{specialist && <p className="mt-2 text-xs text-zinc-400"><span className="text-zinc-600">Preferred creative:</span> {specialist}</p>}</div><div className="flex flex-wrap gap-4">{onChangeSpecialist && <button type="button" onClick={onChangeSpecialist} className="inline-flex min-h-11 w-fit items-center border-b border-white/[0.16] text-sm font-medium text-zinc-300 transition hover:border-orange-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200">Change creative</button>}<button type="button" onClick={onChange} className="inline-flex min-h-11 w-fit items-center border-b border-orange-300/45 text-sm font-medium text-orange-100 transition hover:border-orange-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200">Change service</button></div></section>;
}
...
function ServiceStep({ draft, availableServices, selectService, serviceCategoryRef, loading, errors, copy }) {
  if (loading) return <LoadingState label="Loading available services" compact />;
  return <div ref={serviceCategoryRef} data-flow-step="category" aria-live="polite"><ChoiceGroup fieldKey="serviceKey" legend={copy.serviceSelectionHeading} error={errors.serviceKey}><p className="mt-2 text-sm leading-6 text-zinc-400">{copy.serviceSelectionDescription}</p><p className="mt-2 text-xs leading-5 text-zinc-600">{copy.serviceHelper}</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{availableServices.map((service) => <ChoiceButton key={`${service.legacyBranch}:${service.key}`} selected={draft.serviceKey === service.key && draft.branch === service.legacyBranch} onClick={() => selectService(service.key)} title={service.name} compact />)}</div></ChoiceGroup></div>;
}
...
function ReviewStep({ draft, service, creative, copy }) {
  const branch = null;
...
function ReviewItem({ label, value, hint = '' }) { if (label === 'Branch') return null; return <div className="border-t border-white/[0.08] pt-3"><p className="text-[10px] uppercase tracking-[0.16em] text-zinc-600">{label}</p>{hint && <p className="mt-1 text-xs leading-5 text-zinc-500">{hint}</p>}<p className="mt-2 text-sm text-zinc-300">{value || 'Not specified'}</p></div>; }
