...
function TourismDetailContent({ type }) {
  ...
  return <article className="min-h-screen bg-zinc-950 pb-20 text-zinc-100">
    ...
    {flags.publicInquiriesEnabled && <section className="page-shell mt-12"><div className="mx-auto max-w-3xl border-t border-white/[0.1] pt-7"><h2 className="text-xl font-semibold">Ask about this {meta.label.toLowerCase()}</h2><p className="mt-2 text-sm text-zinc-400">Send an inquiry with this page already attached as context.</p><Link to={`/inquiry?contextType=${encodeURIComponent(type)}&contextSlug=${encodeURIComponent(slug)}&contextTitle=${encodeURIComponent(post.title)}`} className="mt-5 inline-flex min-h-11 items-center bg-[var(--site-accent)] px-5 text-sm font-semibold text-zinc-950">Inquire</Link></div></section>}
  </article>;
}
