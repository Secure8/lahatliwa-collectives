...
export async function uploadManagedWebsiteImage(file, { category, projectId = '', creativeMemberId = '', editorialPostId = '', creativePostId = '', onStatus, override = false, overrideReason = '' } = {}) {
  ...
  const budget = await invoke({ action: 'check_budget', category, ...(projectId ? { projectId } : {}), ...(creativeMemberId ? { creativeMemberId } : {}), ...(editorialPostId ? { editorialPostId } : {}), ...(creativePostId ? { creativePostId } : {}), estimatedBytes: Number(file?.size || 0), override, overrideReason });
