...
function WorkProject({ project }) {
  ...
  return <article className="grid gap-7 border-b border-[var(--site-divider)] pb-12 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
    <div>{cover && <Link to={`/projects/${project.slug}`}><img src={cover} alt={project.title} loading="lazy" decoding="async" className="aspect-[4/3] w-full rounded-xl object-cover" /></Link>}<p className="public-eyebrow mt-5">In progress · {project.category}</p><h2 className="mt-3 text-3xl font-semibold text-[var(--site-primary-text)]">{project.title}</h2><p className="mt-4 text-sm leading-7 text-[var(--site-secondary-text)]">{project.description}</p><Link to={`/projects/${project.slug}`} className="fine-link mt-6 inline-flex min-h-11 items-center gap-2 text-sm text-[var(--site-primary-text)]">Open project <ArrowRight size={15} /></Link></div>
