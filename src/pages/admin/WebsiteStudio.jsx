...
export default function WebsiteStudio() {
  ...
  async function uploadImage(fieldKey, file) {
    ...
    try {
      const isPageCover = /background|cover/i.test(fieldKey);
      const url = await uploadSiteAsset(file, isPageCover ? 'page-covers' : 'brand', isPageCover ? 'siteImage' : 'siteLogo');
      updateField(fieldKey, url, 'image');
      setNotice(`${isPageCover ? 'Cover image' : 'Logo'} uploaded. Save the draft, then publish it to update its connected placement.`);
