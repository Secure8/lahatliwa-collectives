...
import { ArrowRightLeft, CheckCircle2, ChevronDown, Copy, Eye, Hand, MailCheck, RefreshCw, Trash2, UserPlus, X } from 'lucide-react';
...
export default function AdminInquiries() {
  ...
  const { panelRef: inquiryPanelRef } = useModalDrawer({ open: Boolean(selected) && !deleteState, onClose: () => setSelectedId('') });
  const { panelRef: deletePanelRef } = useModalDrawer({ open: Boolean(selected && deleteState), onClose: () => { if (!working) setDeleteState(null); } });
  ...
  return <AdminLayout><div className="w-full max-w-7xl">
    ...
    <div className="public-filter-scroll flex gap-2 overflow-x-auto border-y border-white/[0.08] py-2" aria-label="Inquiry views">{WORKFLOW_VIEWS.map(([key, label]) => <button key={key} type="button" aria-pressed={view === key} onClick={() => setView(key)} className={`interactive-tab min-h-11 shrink-0 px-3 text-xs uppercase tracking-[0.13em] ${view === key ? 'text-white' : 'text-zinc-500 hover:text-white'}`}>{label}</button>)}</div>
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center overflow-hidden bg-black/80 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-labelledby="inquiry-detail-title"><AdminSurface ref={inquiryPanelRef} className="grid h-dvh max-h-dvh w-full min-w-0 max-w-5xl gap-5 overflow-y-auto overflow-x-hidden rounded-none border-0 bg-zinc-950/98 px-4 pb-[calc(5rem+env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] shadow-2xl sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:gap-6 sm:rounded-md sm:border sm:border-amber-200/20 sm:p-5"><div className="sticky top-0 z-10 -mx-4 -mt-4 flex items-start justify-between gap-4 border-b border-amber-200/15 bg-zinc-950/96 px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur-xl sm:static sm:mx-0 sm:mt-0 sm:bg-transparent sm:px-0 sm:pt-0"><div className="min-w-0"><p className="break-words text-xs uppercase tracking-[0.18em] text-amber-200/70">{selected.public_reference || 'Legacy inquiry'}</p><h2 id="inquiry-detail-title" className="mt-1.5 break-words text-xl font-semibold text-white">{selected.name}</h2></div><button data-drawer-initial-focus type="button" onClick={() => setSelectedId('')} aria-label="Close inquiry details" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl text-zinc-400 hover:bg-white/[0.06] hover:text-white"><X size={20} /></button></div>
      ...
      <section className="rounded-md border border-amber-200/20 bg-amber-300/[0.045] p-4">
        ...
        <div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="shrink-0">{canAcceptInquiry(selected, memberId) ? <AdminButton onClick={() => performAction('accept', {}, { message: 'Inquiry accepted.', scope: 'recommended' })} disabled={working}><CheckCircle2 size={15} />Accept inquiry</AdminButton> : canStartProgress ? <AdminButton onClick={() => performAction('start_progress', {}, { message: 'Inquiry marked in progress.', scope: 'recommended' })} disabled={working}>Start progress</AdminButton> : canFinish ? <AdminButton onClick={() => performAction('mark_completed', { note: actionNote }, { message: 'Inquiry marked completed.', scope: 'recommended' })} disabled={working}><CheckCircle2 size={15} />Mark completed</AdminButton> : canClose ? <AdminButton onClick={() => performAction('close', {}, { message: 'Inquiry closed.', scope: 'recommended' })} disabled={working}>Close inquiry</AdminButton> : canRespond && !currentResponse ? <AdminButton onClick={() => performAction('respond', { response: 'available', note: responseNote }, { message: 'Your availability was shared.', scope: 'recommended' })} disabled={working}>I am available</AdminButton> : null}</div>
        </div>
      ...
      <ExpandableSection title="Team availability" description="Current responses and optional availability notes"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-zinc-400">Responses are independent and do not change public profile availability.</p>{canRespond && <div className="flex flex-wrap gap-2"><AdminActionButton onClick={() => performAction('respond', { response: 'available', note: responseNote })} disabled={working}>Available to take this</AdminActionButton><AdminActionButton onClick={() => performAction('respond', { response: 'declined', note: responseNote })} disabled={working}>Decline</AdminActionButton><AdminActionButton onClick={() => performAction('respond', { response: 'unavailable', note: responseNote })} disabled={working}>Unavailable</AdminActionButton>{currentResponse && <AdminActionButton onClick={() => performAction('respond', { response: 'clear' })} disabled={working}>Clear response</AdminActionButton>}</div>}</div>{canRespond && <input value={responseNote} onChange={(event) => { setResponseNote(event.target.value); if (feedbackScope === 'responses') { setError(''); setMessage(''); } }} placeholder="Optional Team-visible response note" maxLength="2000" className={`${lineControl} mt-4`} />}<div className="mt-4 grid gap-2 sm:grid-cols-2">{responses.map((item) => <div key={item.memberId} className="border-t border-white/[0.07] py-3 text-sm"><span className="text-zinc-200">{item.name}</span><span className="text-zinc-600"> — </span><span className={item.response === 'available' ? 'text-emerald-300' : 'text-zinc-400'}>{RESPONSE_LABELS[item.response] || 'No response'}</span>{item.note && <p className="mt-1 text-xs leading-5 text-zinc-500">{item.note}</p>}</div>)}</div><ScopedFeedback scope="responses" activeScope={feedbackScope} error={error} message={message} /></ExpandableSection>
      {showAssignmentTools && <ExpandableSection title="More workflow options" description="Transfers, assignment requests, and administrative assignment"><div className="grid min-w-0 gap-4 sm:grid-cols-2"><label className="grid min-w-0 gap-1.5 text-sm text-zinc-300"><span>Destination creative</span><select value={targetMemberId} onChange={(event) => { setTargetMemberId(event.target.value); if (feedbackScope === 'workflow') { setError(''); setMessage(''); } }} className={lineControl}><option value="">Choose an active creative</option>{eligibleCreatives.map((member) => <option key={member.id} value={member.id}>{member.display_name}</option>)}</select></label><label className="grid min-w-0 gap-1.5 text-sm text-zinc-300"><span>Optional reason or note</span><input value={actionNote} onChange={(event) => setActionNote(event.target.value)} maxLength="2000" className={lineControl} /></label></div><div className="mt-4 flex flex-wrap gap-2">{canAcceptInquiry(selected, memberId) && <AdminButton variant="ghost" onClick={() => performAction('decline', { note: actionNote }, { message: 'Inquiry returned to the open Team pool.', scope: 'workflow' })} disabled={working}>Decline assignment</AdminButton>}{selected.current_assignee_id === memberId && targetMemberId && <AdminButton variant="ghost" onClick={() => performAction('transfer', { target_member_id: targetMemberId, note: actionNote }, { notifyAssignment: true, message: 'Inquiry transferred and the receiving creative notified.', scope: 'workflow' })} disabled={working}><ArrowRightLeft size={15} />Pass to creative</AdminButton>}{adminUser?.creative_member_id && selected.current_assignee_id !== memberId && <AdminButton variant="ghost" onClick={() => performAction('request_assignment', { note: actionNote }, { message: 'Assignment request submitted.', scope: 'workflow' })} disabled={working}><Hand size={15} />Request to take this inquiry</AdminButton>}{superAdmin && targetMemberId && <AdminButton variant="ghost" onClick={() => performAction('admin_assign', { target_member_id: targetMemberId, note: actionNote }, { notifyAssignment: true, message: 'Creative assigned and notified.', scope: 'workflow' })} disabled={working}><UserPlus size={15} />Assign creative</AdminButton>}</div>{!targetMemberId && (selected.current_assignee_id === memberId || superAdmin) && <p className="mt-3 text-xs text-zinc-500">Choose a destination creative to reveal transfer or assignment actions.</p>}<ScopedFeedback scope="workflow" activeScope={feedbackScope} error={error} message={message} /></ExpandableSection>}
      ...
      {superAdmin && <ExpandableSection title="Advanced Super Admin tools" description="Private notes, archiving, and restricted maintenance"><label className="grid gap-1.5 text-sm text-zinc-300"><span>Private note</span><textarea value={privateNote} onChange={(event) => { setPrivateNote(event.target.value); if (feedbackScope === 'advanced') { setError(''); setMessage(''); } }} maxLength="2000" className={`${lineControl} min-h-24 resize-y`} /></label><div className="mt-4 flex flex-wrap gap-3"><AdminButton variant="ghost" onClick={() => performAction('private_note', { note: privateNote }, { message: 'Private note saved.', scope: 'advanced' })} disabled={working}>Save private note</AdminButton><AdminButton variant="ghost" onClick={() => performAction('archive', {}, { message: 'Inquiry archived.', scope: 'advanced' })} disabled={working}>Archive inquiry</AdminButton></div><ScopedFeedback scope="advanced" activeScope={feedbackScope} error={error} message={message} />{canDeleteInquiry(selected, role) && <div className="mt-6 border-t border-red-300/20 pt-5"><p className="text-xs font-medium uppercase tracking-[0.14em] text-red-200">Destructive action</p><p className="mt-2 text-xs leading-5 text-zinc-500">Permanent deletion is available only for completed or closed inquiries and cannot be undone.</p><AdminButton className="mt-4" variant="danger" onClick={() => { setError(''); setMessage(''); setFeedbackScope('delete'); setDeleteState({ pin: '', confirmation: '' }); }} disabled={working}><Trash2 size={15} />Permanent Delete</AdminButton></div>}</ExpandableSection>}
    </AdminSurface></div>}

    {deleteState && selected && <div className="fixed inset-0 z-[60] grid place-items-end bg-black/85 sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="delete-inquiry-title"><AdminSurface ref={deletePanelRef} className="grid w-full max-w-lg gap-5 rounded-b-none border-red-300/20 bg-zinc-950 pb-[max(1rem,env(safe-area-inset-bottom))] sm:rounded-md"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-red-300/70">Irreversible action</p><h2 id="delete-inquiry-title" className="mt-2 text-xl font-semibold text-white">Permanently delete inquiry</h2></div><button data-drawer-initial-focus type="button" onClick={() => !working && setDeleteState(null)} aria-label="Close permanent deletion dialog" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl hover:bg-white/[0.06]"><X size={20} /></button></div><p className="text-sm leading-6 text-zinc-300">This permanently removes the completed inquiry, responses, receipts, assignment requests, transfer history, notifications, delivery attempts, notes, and status history.</p><AdminInput label="Super Admin security PIN" type="password" value={deleteState.pin} onChange={(value) => { setDeleteState((current) => ({ ...current, pin: value })); setError(''); }} /><AdminInput label="Type DELETE to confirm" value={deleteState.confirmation} onChange={(value) => { setDeleteState((current) => ({ ...current, confirmation: value })); setError(''); }} /><ScopedFeedback scope="delete" activeScope={feedbackScope} error={error} message={message} /><div className="grid grid-cols-2 gap-3">{deleteState.pin && deleteState.confirmation === 'DELETE' && <AdminButton variant="danger" onClick={permanentlyDelete} disabled={working}>Delete permanently</AdminButton>}<AdminButton variant="ghost" onClick={() => setDeleteState(null)} disabled={working}>Cancel</AdminButton></div>{(!deleteState.pin || deleteState.confirmation !== 'DELETE') && <p className="text-xs text-zinc-500">Enter the security PIN and type DELETE to reveal the permanent delete action.</p>}</AdminSurface></div>}
  </div></AdminLayout>;
}
