...
export default function AdminEditorial() {
  ...
  return <AdminLayout><AdminPageHeader eyebrow="Tourism and editorial" title="Editorial module" description="Control release flags, editorial content, homepage composition, taxonomy, contributors, and audit records." action={<><AdminButton to="/editorial" variant="ghost">Studio <ExternalLink size={15} /></AdminButton><AdminButton to="/editorial/new" variant="primary"><Plus size={15} />New</AdminButton></>} /><nav className="mb-6 flex gap-1 overflow-x-auto pb-2" aria-label="Editorial administration">{sections.map(([label, href, Icon]) => <Link key={href} to={href} className={`inline-flex h-10 shrink-0 items-center gap-2 rounded-md border px-3 text-xs font-semibold ${location.pathname === href ? 'border-amber-200/25 bg-amber-200/[0.1] text-amber-100' : 'border-white/[0.08] text-zinc-400 hover:text-white'}`}><Icon size={14} />{label}</Link>)}</nav><AdminEditorialSection section={key} /></AdminLayout>;
}
...
function EditorialSettings() {
  ...
  return <AdminSurface><h2 className="text-lg font-semibold">Release flags</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">All flags default off. Enable the module only after migration, Edge Function, R2 cleanup, and live authorization checks are ready.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><AdminCheckbox label="Module" checked={state.flags.moduleEnabled} onChange={(value) => setFlag('moduleEnabled', value)} /><AdminCheckbox label="Public portal" checked={state.flags.publicPortalEnabled} onChange={(value) => setFlag('publicPortalEnabled', value)} /><AdminCheckbox label="Tourism on home" checked={state.flags.homepageTourismEnabled} onChange={(value) => setFlag('homepageTourismEnabled', value)} /><AdminCheckbox label="Editorial Studio" checked={state.flags.editorialStudioEnabled} onChange={(value) => setFlag('editorialStudioEnabled', value)} /><AdminCheckbox label="Public inquiries" checked={state.flags.publicInquiriesEnabled} onChange={(value) => setFlag('publicInquiriesEnabled', value)} /><AdminCheckbox label="Media uploads" checked={state.flags.editorialMediaUploadsEnabled} onChange={(value) => setFlag('editorialMediaUploadsEnabled', value)} /></div><div className="mt-5 flex items-center gap-4"><AdminButton onClick={save} variant="primary">Save</AdminButton>{state.error && <AdminNotice className="flex-1">{state.error}</AdminNotice>}{state.message && <AdminNotice tone="success" className="flex-1">{state.message}</AdminNotice>}</div></AdminSurface>;
}
...
function TaxonomyManager({ section }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[20rem_1fr]"><AdminSurface><h2 className="font-semibold capitalize">Add {section === 'categories' ? 'category' : section.slice(0, -1)}</h2><form onSubmit={add} className="mt-4 grid gap-4"><AdminInput label="Name" value={state.name} onChange={(name) => setState((current) => ({ ...current, name }))} required /><AdminButton type="submit" variant="primary">Add</AdminButton></form>{state.error && <AdminNotice className="mt-4">{state.error}</AdminNotice>}</AdminSurface><AdminSurface><h2 className="font-semibold capitalize">{section}</h2><div className="mt-4 divide-y divide-white/[0.08]">{state.rows.map((row) => <div key={row.id} className="flex items-center justify-between gap-4 py-3"><div><p className="text-sm font-medium">{row.name || row.display_name}</p><p className="mt-1 text-xs text-zinc-500">{row.slug}</p></div>{Object.hasOwn(row, 'is_active') && <AdminStatusBadge status={row.is_active ? 'active' : 'disabled'} />}</div>)}{!state.rows.length && <p className="py-8 text-sm text-zinc-500">No records.</p>}</div></AdminSurface></div>;
}
