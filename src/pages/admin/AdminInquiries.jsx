...
import { AdminActionButton, AdminButton, AdminInput, AdminNotice, AdminStatusBadge } from '../../components/admin/AdminUI';
...
export default function AdminInquiries() {
  ...
  return <AdminLayout><div className="ll-inquiry-tool">
    <header className="ll-operations-intro"><p className="ll-kicker">Messages</p><h2>Inquiries</h2><p>Review requests sent to the platform. Creative-specific inquiries stay with the Creative who received them.</p></header>
    ...
    <div className="ll-inquiry-toolbar" data-inquiry-filter-panel>
      <label className="ll-simple-search"><Eye size={17}/><span className="sr-only">Search inquiries</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search inquiries" /></label>
      <label className="ll-inquiry-view"><span className="sr-only">Inquiry view</span><select value={view} onChange={(event) => setView(event.target.value)}>{WORKFLOW_VIEWS.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
      <button type="button" aria-pressed={showArchived} onClick={() => setShowArchived((current) => !current)} className="ll-archive-toggle">{showArchived ? 'Show inbox' : 'Archived'}</button>
    </div>
    ...
    })}</div> : <div className="ll-operations-empty"><strong>No inquiries match</strong><p>Try another search or view, or wait for a new message.</p></div>}
