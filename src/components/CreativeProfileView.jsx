  ...
  return (
    <article className="min-w-0">
      ...
      {!adminPreview && <section className="pt-10">
        ...
        {projects.length ? <div className="mt-7 grid gap-x-5 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">{projects.map((project) => <ProfileProject key={project.id} project={project} adminPreview={adminPreview} />)}</div> : <p className="mt-7 border-t border-white/[0.07] pt-6 text-sm text-zinc-500">{adminPreview ? 'No linked projects yet.' : 'Credited works will appear here when they are published.'}</p>}
      </section>}
    </article>
  );
}
...
function ProfileProject({ project, adminPreview }) {
  ...
  const imageContent = image ? <img src={image} alt={project.title} loading="lazy" decoding="async" width="800" height="600" className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:opacity-85" /> : <div className="grid aspect-[4/3] place-items-center px-5 text-center text-sm text-zinc-500">Open project</div>;
  ...
  return <article className="group"><Link to={`/projects/${project.slug}`} className="block bg-zinc-900">{imageContent}</Link><Link to={`/projects/${project.slug}`} className="mt-3 block">{content}</Link></article>;
}
