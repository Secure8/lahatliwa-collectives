...
test('project and profile permissions allow Super Admin or the owning Creative only', () => {
  ...
  assert.equal(r2ProjectPermissionAllowed({ role: 'super_admin', userId: 'admin', project: { ...project, status: 'published' } }, 'delete'), true);
  assert.equal(r2ProjectPermissionAllowed({ role: 'creative', userId: 'owner', project: { ...project, status: 'published' } }, 'delete'), true);
  assert.equal(r2ProjectPermissionAllowed({ role: 'creative', userId: 'other', project }, 'delete'), false);
