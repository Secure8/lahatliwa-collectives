...
export default function Discover() {
  ...
  return <div className="ll-discover-page page-shell">
    ...
    <section className="ll-discover-filters" aria-label="Filter Creative work">
      ...
      <div>{['discipline','specialty','industry'].map((kind) => <label key={kind}><span className="sr-only">{kind}</span><select value={filters[kind]} onChange={(event) => update(kind, event.target.value)}><option value="">All {{ discipline: 'disciplines', specialty: 'specialties', industry: 'industries' }[kind]}</option>{(grouped[kind] || []).map((term) => <option key={term.id} value={term.slug}>{term.name}</option>)}</select></label>)}</div>
    </section>
