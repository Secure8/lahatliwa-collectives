    ...
    <AdminLayout>
      ...
      <AdminSurface as="form" onSubmit={save} className="grid gap-5">
        ...
        <div className="grid gap-3 border-y border-white/[0.07] py-5">
          <div className="flex flex-wrap items-center gap-3"><label className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-white/[0.055] px-3 py-2 text-sm text-zinc-200 ring-1 ring-white/[0.08]"><ImagePlus size={15} />{form.cover_image ? 'Replace cover photo' : 'Add cover photo'}<input className="sr-only" disabled={uploading || saving} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { uploadImage(event.target.files?.[0], 'cover'); event.target.value = ''; }} /></label>{form.cover_image && <AdminButton type="button" variant="danger" disabled={uploading || saving} onClick={() => removeImage('cover_image', 'cover photo')}><Trash2 size={15} /> Remove</AdminButton>}<span className="text-xs text-zinc-500">Cover images are resized to 1800px and optimized to 1 MB.</span></div>
