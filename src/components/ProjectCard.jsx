  ...
  return (
    <article className="group">
      ...
      <div className="border-b border-white/[0.07] py-5">
        ...
        <Link to={`/projects/${project.slug}`} className="mt-2 flex items-start justify-between gap-4 text-white">
          <h3 className="text-xl font-medium leading-snug">{project.title}</h3>
          <ArrowUpRight className="mt-1 shrink-0 text-zinc-500 transition group-hover:text-amber-200" size={17} />
        </Link>
        <p className="mt-2 max-w-xl text-sm leading-6 text-zinc-400">{excerpt(project.description, 110)}</p>
      </div>
    </article>
  );
}
