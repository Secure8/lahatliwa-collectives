...
export default function CreativeCard({ creative, headingLevel = 'h3' }) {
  ...
  return (
    <article id={`creative-${creative.id}`} className="mobile-app-card group relative flex h-full scroll-mt-24 flex-col border-t border-white/[0.09] pt-5 after:absolute after:left-0 after:top-[-1px] after:h-px after:w-0 after:bg-orange-300 after:shadow-[0_0_12px_rgba(253,186,116,0.75)] after:transition-all after:duration-500 hover:after:w-20">
      <Link to={`/creatives/${creative.slug}`} state={linkState} onPointerEnter={() => preloadPublicRoute('/creatives/:slug')} onFocus={() => preloadPublicRoute('/creatives/:slug')} aria-label={`View ${creative.name}`} className="flex h-full flex-col rounded-xl transition duration-500 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300 motion-reduce:transform-none motion-reduce:transition-none">
        ...
        <div className="site-primary mt-2 flex min-h-11 items-start justify-between gap-4 border-b border-white/[0.12] pb-2 transition group-hover:border-orange-300/50 group-hover:text-orange-100">
