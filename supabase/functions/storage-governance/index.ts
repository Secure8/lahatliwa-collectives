...
async function providerStorageUsage(actor:any){
  ...
  const r2Failure=r2Usage.status==='rejected'?(r2Usage.reason as {code?:string;bucket?:string;pagesScanned?:number;objectsScannedBeforeFailure?:number;checkedAt?:string}):null;
  return{
    supabase:supabaseData?{...supabaseData,available:true}:{available:false,complete:false,code:'SUPABASE_USAGE_UNAVAILABLE',source:'current_storage_objects_in_this_project',checkedAt},
    r2:r2Usage.status==='fulfilled'?r2Usage.value:{available:false,complete:false,code:String(r2Failure?.code||'R2_USAGE_UNAVAILABLE').slice(0,80),bucket:r2Failure?.bucket||(config.configured?config.bucketName:null),pagesScanned:Number(r2Failure?.pagesScanned||0),objectsScannedBeforeFailure:Number(r2Failure?.objectsScannedBeforeFailure||0),source:'r2_list_objects_v2',checkedAt:r2Failure?.checkedAt||checkedAt},
  };
}
Deno.serve(async(request)=>{const env=edgeEnvironment();const cors=corsHeaders(request,env.siteOrigin);if(request.method==='OPTIONS')return new Response(null,{status:204,headers:cors});if(request.method!=='POST')return fail('METHOD_NOT_ALLOWED','Method not allowed.',405,cors);if(!cors['Access-Control-Allow-Origin'])return fail('ORIGIN_NOT_ALLOWED','This request origin is not allowed.',403,cors);const actor=await authenticatedTeamMember(request,env);if('error'in actor)return fail(String(actor.error||'INVALID_SESSION'),'Only an active Super Admin can access storage governance.',Number(actor.status||401),cors);if(actor.role!=='super_admin')return fail('NOT_AUTHORIZED','Only the Super Admin can access storage governance.',403,cors);const body=cleanBody(await request.json().catch(()=>({})));try{
  if(body.action==='dashboard'){const [{data,error},providerUsage]=await Promise.all([actor.admin.rpc('get_storage_governance_snapshot'),providerStorageUsage(actor)]);if(error)throw error;return reply({success:true,snapshot:{...(data||{}),providerUsage,refresh:{forced:body.forceRefresh===true,cache:'no-store'}}},200,cors);}
