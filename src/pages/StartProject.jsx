...
function SelectionSummary({ branch, service, specialist = '', onChange, onChangeSpecialist }) {
  return <section aria-label="Inquiry selections" className="mt-8 flex flex-col gap-4 border-y border-white/[0.09] py-4 sm:flex-row sm:items-center sm:justify-between"><div aria-live="polite" aria-atomic="true"><p className="text-[10px] uppercase tracking-[0.18em] text-orange-300">Selected service</p><p className="mt-1 text-sm font-medium text-white">{branch} <span aria-hidden="true" className="text-zinc-600">·</span> {service}</p>{specialist && <p className="mt-2 text-xs text-zinc-400"><span className="text-zinc-600">Preferred creative:</span> {specialist}</p>}</div><div className="flex flex-wrap gap-4">{onChangeSpecialist && <button type="button" onClick={onChangeSpecialist} className="inline-flex min-h-11 w-fit items-center border-b border-white/[0.16] text-sm font-medium text-zinc-300 transition hover:border-orange-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200">Change preferred creative</button>}<button type="button" onClick={onChange} className="inline-flex min-h-11 w-fit items-center border-b border-orange-300/45 text-sm font-medium text-orange-100 transition hover:border-orange-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200">Change selection</button></div></section>;
}
