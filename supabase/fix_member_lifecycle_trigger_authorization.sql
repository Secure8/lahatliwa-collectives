-- Narrow transaction-local authorization for the service-role-only member
-- lifecycle RPC. Normal browser updates continue through both original guards.
...
begin
  if coalesce(current_setting('app.admin_member_lifecycle_authorized', true), '')='true' and auth.role()='service_role' then return new; end if;
...
returns trigger language plpgsql security definer set search_path=public,private,pg_temp as $$
declare requester_id uuid := auth.uid(); requester_email text := lower(auth.jwt()->>'email');
begin
  if coalesce(current_setting('app.admin_member_lifecycle_authorized', true), '')='true' and auth.role()='service_role' then return new; end if;
  if requester_id is null then raise exception 'Authentication required.'; end if;
  if private.has_role(requester_id,array['super_admin','admin']) then return new; end if;
  if requester_email=lower(old.email) and old.status in ('invited','active') and old.user_id is null and new.user_id=requester_id and lower(new.email)=lower(old.email) then
    new.id:=old.id; new.email:=old.email; new.role:=old.role;
    new.status:=case when old.status='invited' then 'active' else old.status end;
    new.creative_member_id:=old.creative_member_id; new.invited_by:=old.invited_by; return new;
  end if;
  if old.user_id=requester_id then
    new.id:=old.id; new.user_id:=old.user_id; new.email:=old.email; new.role:=old.role;
    new.status:=old.status; new.creative_member_id:=old.creative_member_id; new.invited_by:=old.invited_by; return new;
  end if;
  raise exception 'Only admins can change team access fields.';
end; $$;

create or replace function public.execute_admin_member_lifecycle(p_action text,p_target_admin_user_id uuid,p_actor_user_id uuid)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare
  target public.admin_users%rowtype; active_super_admins integer;
  snapshot public.admin_member_lifecycle_snapshots%rowtype; linked_project_ids uuid[];
  project_id uuid; remaining_credits integer;
begin
  ...
  if not exists(select 1 from public.admin_users where user_id=p_actor_user_id and status='active' and role in ('super_admin','owner')) then raise exception 'Only an active Super Admin may perform this action'; end if;
  ...
  if p_action='remove_access' then
    ...
    select coalesce(array_agg(pc.project_id),'{}'::uuid[]) into linked_project_ids from public.project_creatives pc where coalesce(pc.creative_member_id,pc.creative_id)=target.creative_member_id;
    ...
    values(target.id,target.status,target.creative_member_id,(select is_published from public.creative_members where id=target.creative_member_id),coalesce((select jsonb_agg(jsonb_build_object('id',p.id,'status',p.status,'review_status',p.review_status)) from public.projects p where p.id=any(linked_project_ids)),'[]'::jsonb),p_actor_user_id)
    on conflict(admin_user_id) do update set previous_status=excluded.previous_status,creative_member_id=excluded.creative_member_id,creative_was_published=excluded.creative_was_published,project_states=excluded.project_states,removed_at=now(),removed_by=excluded.removed_by;
  ...
  if p_action='restore_access' then
    ...
    update public.projects p set status=s.value->>'status',review_status=coalesce(s.value->>'review_status','draft'),updated_at=now() from jsonb_array_elements(snapshot.project_states) s where p.id=(s.value->>'id')::uuid;
  ...
  select coalesce(array_agg(distinct pc.project_id),'{}'::uuid[]) into linked_project_ids from public.project_creatives pc where coalesce(pc.creative_member_id,pc.creative_id)=target.creative_member_id;
  ...
  update public.admin_users set user_id=null,email='deleted-'||id::text||'@invalid.local',display_name='Deleted member',avatar_url=null,creative_member_id=null,status='deleted',updated_at=now() where id=target.id;
