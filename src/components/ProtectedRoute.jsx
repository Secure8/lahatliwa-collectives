...
function AccessBlocked({ title, message, email }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-zinc-950 px-4 py-12 text-white"><section className="w-full max-w-lg"><ThemeControl className="mb-5" /><div className="border-y border-white/[0.1] py-8"><p className="text-xs uppercase tracking-[0.2em] text-zinc-600">Access check</p><h1 className="mt-3 text-2xl font-semibold">{title}</h1><p className="mt-4 text-sm leading-7 text-zinc-400">{message}</p>{email && <p className="mt-3 text-sm text-zinc-500">Signed in as <span className="text-zinc-300">{email}</span></p>}<div className="mt-6 flex flex-wrap gap-4"><button type="button" onClick={signOut} className="border-b border-amber-200/40 pb-1 text-sm text-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/50">Sign out</button><Link to="/" className="border-b border-white/[0.12] pb-1 text-sm text-zinc-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/50">Return to public site</Link></div></div></section></main>;
}
