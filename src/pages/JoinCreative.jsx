...
export default function JoinCreative() {
  ...
  async function submit(event) {
    ...
    const { data, error: requestError } = await supabase.rpc('submit_creative_join_request', {
      p_name: form.name,
      p_email: form.email,
    ...
    else if (!data) setError('Your request could not be confirmed. Please try again.');
    else { setRequestId(String(data)); setForm(emptyForm); }
    setSaving(false);
  }
  ...
  return <main className="ll-auth-page"><section className="ll-auth-card ll-join-card">
    ...
    {requestId ? <div className="ll-auth-form ll-join-success"><CheckCircle2 size={28}/><h1>Request received</h1><p>A Super Admin will review your request. If approved, you’ll receive an email to create your Creative account.</p><small>Request {requestId.slice(0, 8).toUpperCase()}</small><Link to="/" className="ll-primary-action">Return to the feed</Link></div> : <form onSubmit={submit} className="ll-auth-form">
