import {
  ...
  r2CreativePostPermissionAllowed,
...
const MEDIA_FIELDS = 'id,owner_user_id,provider,external_file_id,filename,mime_type,size_bytes,trusted_size_bytes,width,height,status,file_category,project_id,creative_member_id,editorial_post_id,creative_post_id,media_group_id,media_variant,public_url,reservation_id,verification_status,accounting_state,cleanup_status,cleanup_attempt_count,cleanup_error,metadata,created_at,updated_at';
...
async function authorizeCreativePostEdit(actor: any, creativePostId: string) {
  const { data: post, error } = await actor.admin.from('creative_posts')
    .select('id,status,moderation_status,author_user_id,creative_member_id').eq('id', creativePostId).maybeSingle();
  if (error) throw Object.assign(new Error('Creative post media authorization failed'), { code: 'REFERENCE_CHECK_FAILED' });
  return r2CreativePostPermissionAllowed({ role: actor.role, userId: actor.user.id,
    creativeMemberId: actor.teamMember.creative_member_id, post }) ? post : null;
}

async function authorizeTarget(actor: any, category: string, projectId = '', creativeMemberId = '', editorialPostId = '', creativePostId = '') {
  ...
  if (definition.target === 'creative_post') return authorizeCreativePostEdit(actor, creativePostId);
...
async function targetReferences(actor: any, row: any, url: string) {
  ...
  if (row.creative_post_id) {
    const results = await Promise.all([
      actor.admin.from('creative_posts').select('document').eq('id', row.creative_post_id).maybeSingle(),
      actor.admin.from('creative_post_media').select('media_group_id').eq('post_id', row.creative_post_id),
    ]);
    if (results.some((result: any) => result.error)) throw Object.assign(new Error('Creative post reference verification failed'), { code: 'REFERENCE_CHECK_FAILED' });
    return results.some((result: any) => containsReference(result.data, url))
      || (results[1].data || []).some((item: any) => item.media_group_id === row.media_group_id);
  }
  const results = await Promise.all([
...
Deno.serve(async (request) => {
  ...
  if (body.action === 'check_budget') {
    if (!only(body, ['action','category','projectId','creativeMemberId','editorialPostId','creativePostId','estimatedBytes','override','overrideReason'])) return fail('INVALID_REQUEST', 'The storage check request is invalid.', 400, cors);
    const category = String(body.category || '');
    if (!R2_MEDIA_CATEGORIES[category] || !await authorizeTarget(actor, category, String(body.projectId || ''), String(body.creativeMemberId || ''), String(body.editorialPostId || ''), String(body.creativePostId || ''))) return fail('TARGET_NOT_AUTHORIZED', 'You cannot upload media to this destination.', 403, cors);
  ...
  if (body.action === 'initiate') {
    if (!only(body, ['action','category','projectId','creativeMemberId','editorialPostId','creativePostId','variants','override','overrideReason'])) return fail('INVALID_REQUEST', 'The media upload request contains unsupported fields.', 400, cors);
    ...
    if (!await authorizeTarget(actor, validation.categoryKey, validation.projectId, validation.creativeMemberId, validation.editorialPostId, validation.creativePostId)) return fail('TARGET_NOT_AUTHORIZED', 'You do not have permission to upload media here.', 403, cors);
    ...
    const targetId = validation.projectId || validation.creativeMemberId || validation.editorialPostId || validation.creativePostId || actor.user.id;
    const rows = validation.variants.map((variant: any) => {
      ...
      return {
        ...
        creative_post_id: validation.creativePostId || null,
  ...
  if (body.action === 'commit_replacement') {
    ...
    if (!await authorizeTarget(actor, rows[0].file_category, rows[0].project_id, rows[0].creative_member_id, rows[0].editorial_post_id, rows[0].creative_post_id)) return fail('TARGET_NOT_AUTHORIZED', 'You do not have permission to finish this replacement.', 403, cors);
  ...
  if (body.action === 'request_delete') {
    ...
    if (!await authorizeTarget(actor, row.file_category, row.project_id, row.creative_member_id, row.editorial_post_id, row.creative_post_id)) return fail('TARGET_NOT_AUTHORIZED', 'You do not have permission to remove this media.', 403, cors);
