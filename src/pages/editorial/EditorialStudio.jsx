...
const DETAIL_FIELDS = Object.freeze({
  ...
  place: [
    ['address_text', 'Address'], ['place_type', 'Place type'], ['opening_hours_note', 'Opening information'],
...
function EditorialStudioShell() {
  ...
  return <div className="min-h-screen overflow-x-clip bg-[#090908] text-zinc-100">
    <header className="sticky top-0 z-50 border-b border-white/[0.08] bg-[#090908]/95 backdrop-blur-xl">
    ...
    {editorMode ? <main className="min-w-0">{content}</main> : <div className="mx-auto grid max-w-[110rem] lg:grid-cols-[14rem_minmax(0,1fr)]">
      <aside className={clsx('border-b border-white/[0.08] bg-[#0c0c0b] px-3 py-3 lg:sticky lg:top-16 lg:block lg:h-[calc(100vh-4rem)] lg:border-b-0 lg:border-r lg:py-5', menuOpen ? 'block' : 'hidden')}>
...
function StudioLibrary() {
  ...
  async function load() {
    try {
      ...
      setState((current) => ({ ...current, loading: false, error: 'Your content library could not be loaded.', posts: [] }));
    }
  }
...
function StoryEditor({ id }) {
  ...
  return <section className="editorial-studio-page pb-24 md:pb-0">
    ...
    <div className={clsx('grid min-h-[calc(100vh-8rem)]', editorColumns)}>
      ...
      <main className="min-w-0 bg-[#0d0d0b] px-3 py-6 sm:px-6 lg:px-8">
        <div className={clsx('mx-auto transition-[max-width] duration-300', canvasWidth)}>
          <article className="overflow-hidden rounded-[1.75rem] border border-white/[0.09] bg-[#151512] shadow-[0_30px_100px_rgba(0,0,0,0.35)]">
            ...
            <div className="px-5 py-7 sm:px-10 sm:py-10">
              ...
              <div className="mt-10 space-y-4">{blocks.map((block, index) => <div key={block.id} className={clsx('group/block relative rounded-2xl border p-3 transition', selected === block.id ? 'border-orange-300/45 bg-orange-400/[0.035]' : 'border-transparent hover:border-white/[0.08]', block.hidden && 'opacity-50')} onClick={() => setSelected(block.id)}>
                ...
                <button type="button" onClick={(event) => { event.stopPropagation(); const next = insertEditorialBlock(blocks, 'paragraph', index + 1); updateDocument(next); setSelected(next[index + 1].id); }} className="absolute -bottom-3 left-1/2 z-10 hidden h-7 -translate-x-1/2 items-center gap-1 rounded-full border border-white/[0.12] bg-[#1c1c18] px-2 text-[0.65rem] text-zinc-300 shadow-lg group-hover/block:inline-flex focus:inline-flex"><Plus size={12} />Add Section</button>
...
function EditorToolbar({ post, saveState, canUndo, canRedo, device, setDevice, onUndo, onRedo, onSave, onPreview, onPublish, canPublish, working }) {
  return <header className="sticky top-16 z-40 flex min-h-16 items-center gap-2 border-b border-white/[0.08] bg-[#10100e]/95 px-3 backdrop-blur-xl sm:px-5">
...
function StructurePanel({ collapsed = false, onCollapse, blocks, selected, setSelected, onInsert, onMove, onDuplicate, onToggleCollapse, onDelete, draggedIndex, detailLabel = '', className = '' }) {
  if (collapsed) return <aside className={clsx('sticky top-32 h-[calc(100vh-8rem)] border-r border-white/[0.08] bg-[#0b0b0a] p-2', className)}><IconButton label="Expand story structure" onClick={onCollapse}><Layers3 size={18} /></IconButton></aside>;
  return <aside className={clsx('h-full bg-[#0b0b0a] xl:sticky xl:top-32 xl:h-[calc(100vh-8rem)] xl:overflow-y-auto xl:border-r xl:border-white/[0.08]', className)}>
...
function InspectorPanel({ collapsed = false, onCollapse, selected, selectedBlock, selectedIndex, post, taxonomy, capabilities, flags, uploading, onUploadCover, onPostChange, onBlockPatch, onMove, onDuplicate, onDelete, onChangePost, className = '' }) {
  if (collapsed) return <aside className={clsx('sticky top-32 h-[calc(100vh-8rem)] border-l border-white/[0.08] bg-[#0b0b0a] p-2', className)}><IconButton label="Expand design controls" onClick={onCollapse}><LayoutPanelLeft size={18} /></IconButton></aside>;
  return <aside className={clsx('h-full bg-[#0b0b0a] xl:sticky xl:top-32 xl:h-[calc(100vh-8rem)] xl:overflow-y-auto xl:border-l xl:border-white/[0.08]', className)}>
    ...
    <div className="space-y-5 p-4">{selected === 'cover' && <CoverInspector post={post} flags={flags} uploading={uploading} onUploadCover={onUploadCover} onChange={onPostChange} />}{selected === 'location' && <LocationInspector post={post} taxonomy={taxonomy} onChange={onPostChange} />}{selected === 'story' && <StoryInspector post={post} onChange={onPostChange} />}{selected === 'details' && <DetailsEditor post={post} onChangePost={onChangePost} />}{selected === 'sources' && <SourcesEditor post={post} canManage={capabilities.canManageSources} onChangePost={onChangePost} />}{selectedBlock && <BlockInspector block={selectedBlock} index={selectedIndex} count={post.revision?.document?.blocks?.length || 0} onPatch={onBlockPatch} onMove={onMove} onDuplicate={onDuplicate} onDelete={onDelete} />}</div>
  </aside>;
}
...
function StoryInspector({ post, onChange }) {
  return <InspectorGroup label="Advanced"><StudioInput label="Slug" value={post.slug || ''} onChange={(slug) => onChange('slug', slug)} /><p className="text-xs leading-5 text-zinc-600">The slug was generated from the title. Change it only when the public URL must differ.</p></InspectorGroup>;
}
...
function StoryPreview({ id }) {
  ...
  const { post } = state;
  return <article className="mx-auto max-w-6xl pb-16"><Link to={`/editorial/content/${id}/edit`} className="inline-flex items-center gap-2 text-sm text-zinc-400"><ArrowLeft size={16} />Back to Editor</Link><div className="mt-7 overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[#151512]">{post.cover_image_url ? <img src={post.cover_image_url} alt={post.cover_image_alt || ''} className="aspect-[16/7] w-full object-cover" /> : <div className="grid aspect-[16/7] place-items-center bg-white/[0.025] text-zinc-600"><ImageIcon size={30} /></div>}<header className="mx-auto max-w-3xl px-5 py-10 sm:px-8"><div className="flex flex-wrap gap-2"><Status status={post.status} /><span className="text-xs text-orange-200/70">{contentTypeMeta(post.content_type).label}</span></div><h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-6xl">{post.title}</h1><p className="mt-5 text-lg leading-8 text-zinc-400">{post.summary}</p></header><EditorialDocumentRenderer document={post.revision?.document} mode="preview" className="px-5 pb-10 sm:px-8" />{DETAIL_FIELDS[post.content_type] && <div className="mx-auto max-w-3xl px-5 pb-8 sm:px-8"><div className="rounded-2xl border border-white/[0.08] p-5"><p className="text-xs uppercase tracking-[0.15em] text-orange-200/70">{contentTypeMeta(post.content_type).label} details</p><DetailsPreview type={post.content_type} details={post.details} /></div></div>}{post.sources?.some((source) => source.verification_status === 'verified') && <div className="mx-auto max-w-3xl px-5 pb-12 sm:px-8"><h2 className="text-lg font-semibold">Sources</h2><ul className="mt-3 space-y-2">{post.sources.filter((source) => source.verification_status === 'verified').map((source) => <li key={source.id}><a href={source.source_url} target="_blank" rel="noreferrer" className="text-sm text-orange-200 hover:underline">{source.source_name}</a></li>)}</ul></div>}</div></article>;
}

function MobileEditorToolbar({ onLeft, onRight, onAdd, onSave, onPreview, working }) {
  return <nav className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-5 rounded-2xl border border-white/[0.1] bg-[#171714]/95 p-1.5 shadow-2xl backdrop-blur-xl md:hidden" aria-label="Editor actions"><button onClick={onLeft} className="grid min-h-12 place-items-center rounded-xl text-zinc-300" aria-label="Story structure"><Layers3 size={18} /></button><button onClick={onAdd} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[0.62rem] text-zinc-300"><Plus size={18} />Add</button><button onClick={onSave} disabled={Boolean(working)} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl bg-orange-400 text-[0.62rem] font-semibold text-zinc-950"><Save size={18} />Save</button><button onClick={onPreview} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[0.62rem] text-zinc-300"><Eye size={18} />Preview</button><button onClick={onRight} className="grid min-h-12 place-items-center rounded-xl text-zinc-300" aria-label="Design controls"><LayoutPanelLeft size={18} /></button></nav>;
}

function MobileDrawer({ open, side, title, onClose, children }) {
  if (!open) return null;
  return <div className="fixed inset-0 z-[70] xl:hidden"><button type="button" className="absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={onClose} aria-label="Close panel" /><section className={clsx('absolute inset-y-0 w-[min(90vw,23rem)] overflow-y-auto border-white/[0.1] bg-[#0b0b0a] shadow-2xl', side === 'left' ? 'left-0 border-r' : 'right-0 border-l')} role="dialog" aria-modal="true" aria-label={title}><div className="sticky top-0 z-10 flex min-h-16 items-center justify-between border-b border-white/[0.08] bg-[#0b0b0a]/95 px-4 backdrop-blur"><h2 className="font-semibold">{title}</h2><IconButton label="Close" onClick={onClose}><X size={18} /></IconButton></div>{children}</section></div>;
}

function EmptyLibrary() { return <div className="rounded-2xl border border-dashed border-white/[0.1] bg-white/[0.02] p-12 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-orange-400/10 text-orange-200"><BookOpen size={21} /></span><h2 className="mt-5 font-semibold">No matching stories</h2><p className="mt-2 text-sm text-zinc-500">Adjust the filters or create the first piece of content.</p><Link to="/editorial/new" className="mt-6 inline-flex h-10 items-center gap-2 rounded-full bg-orange-400 px-4 text-sm font-semibold text-zinc-950"><Plus size={15} />New Content</Link></div>; }
...
function StudioInput({ label, value, onChange, required, className = '' }) { return <label className={clsx('grid gap-2 text-sm', className)}><span className="font-medium text-zinc-300">{label}</span><input required={required} value={value ?? ''} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl border border-white/[0.1] bg-black/30 px-3 outline-none focus:border-orange-300/40 focus:ring-2 focus:ring-orange-300/10" /></label>; }
function StudioTextarea({ label, value, onChange, className = '' }) { return <label className={clsx('grid gap-2 text-sm', className)}><span className="font-medium text-zinc-300">{label}</span><textarea rows={3} value={value ?? ''} onChange={(event) => onChange(event.target.value)} className="rounded-xl border border-white/[0.1] bg-black/30 px-3 py-2.5 leading-6 outline-none focus:border-orange-300/40 focus:ring-2 focus:ring-orange-300/10" /></label>; }
function StudioSelect({ label, value, options, onChange }) { return <label className="grid gap-2 text-sm"><span className="font-medium text-zinc-300">{label}</span><select value={value ?? ''} onChange={(event) => onChange(event.target.value)} className="h-11 min-w-0 rounded-xl border border-white/[0.1] bg-black/30 px-3 capitalize outline-none focus:border-orange-300/40">{options.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></label>; }
function CompactSelect({ label, value, options, onChange }) { return <label><span className="sr-only">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="h-11 w-full rounded-xl border border-white/[0.1] bg-black/30 px-3 text-sm outline-none focus:border-orange-300/40">{options.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></label>; }
