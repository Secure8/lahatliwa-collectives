...
export default function Home() {
  ...
  const structuredData = { '@context': 'https://schema.org', '@type': 'WebSite', name: 'Explore Aklan by Lahat Liwa Collectives', url: 'https://www.lahatliwa.studio/', description: 'Independent community stories, destinations, events, activities, and local products across Aklan.', publisher: { '@type': 'Organization', name: 'Lahat Liwa Collectives' } };
  ...
  return <div data-explore-aklan-homepage className="bg-[var(--theme-page-surface)]">
    ...
    <section className="page-shell py-16 sm:py-20" aria-labelledby="featured-creatives-heading">
      <div className="mb-10 flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="max-w-3xl"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--site-accent-text)]">Featured Creatives</p><h2 id="featured-creatives-heading" className="mt-4 text-4xl font-semibold tracking-[-0.03em] text-[var(--site-primary-text)] sm:text-5xl">Meet the people telling Aklan’s stories.</h2><p className="mt-5 text-base leading-7 text-[var(--site-secondary-text)]">Explore published profiles, skills, and credited work from the collective.</p></div><Link to="/creatives" className="fine-link inline-flex min-h-11 shrink-0 items-center gap-2 text-sm font-semibold text-[var(--site-primary-text)]">View Creatives <ArrowRight size={16} /></Link></div>
    ...
    <section className="page-shell py-16 sm:py-20" aria-labelledby="home-inquiry-heading">
      <div className="grid gap-8 border-y border-white/[0.1] py-12 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--site-accent-text)]">Questions and collaborations</p><h2 id="home-inquiry-heading" className="mt-4 max-w-3xl text-3xl font-semibold tracking-[-0.025em] text-[var(--site-primary-text)] sm:text-4xl">Need help finding the right place, story, or creative service?</h2><p className="mt-4 max-w-2xl text-base leading-7 text-[var(--site-secondary-text)]">Choose a tourism question, a creative or digital service, or a general inquiry. We’ll guide you one step at a time.</p></div><Link to="/inquiry?path=general" className="inline-flex min-h-12 w-fit items-center gap-2 bg-[var(--site-accent)] px-5 text-sm font-semibold text-zinc-950 transition hover:bg-[var(--site-accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">Ask a Question <ArrowRight size={17} /></Link></div>
    </section>
  </div>;
}
