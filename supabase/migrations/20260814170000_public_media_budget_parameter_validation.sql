...
-- Keep the R2 budget check explicit about the authenticated actor and the
-- requested operation. Existing valid upload calls retain identical results.
...
begin
  ...
  if p_actor_user_id is null then raise exception 'Media actor is required.' using errcode='22023'; end if;
  if nullif(trim(coalesce(p_operation_kind,'')),'') is null then raise exception 'Media operation is required.' using errcode='22023'; end if;
