...
export default function AdminInquiries() {
  ...
  return <AdminLayout><div className="w-full max-w-6xl">
    ...
    <section className="grid gap-4 border-y border-white/[0.08] py-5 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_10rem_12rem_10rem_auto] xl:items-end">
      <label className="grid gap-1.5 text-sm text-zinc-300"><span>Search</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Reference, client, category, or creative" className={lineControl} /></label>
      ...
      <label className="grid gap-1.5 text-sm text-zinc-300"><span>Service category</span><select value={serviceFilter} onChange={(event) => setServiceFilter(event.target.value)} className={lineControl}><option value="all">All categories</option>{serviceOptions.map((service) => <option key={service} value={service}>{service}</option>)}</select></label>
