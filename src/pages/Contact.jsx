  ...
  return (
    <div className="page-shell py-20">
      ...
      <section className="grid gap-12 pt-10 lg:grid-cols-[1fr_0.72fr] lg:pt-12">
        <div>
          <p className="max-w-lg text-sm leading-7 text-zinc-400">Send one open message for a project idea, collaboration, event, content request, profile or credit question, opportunity, or general concern. You do not need to choose a service first.</p>
          <div className="mt-7 flex flex-wrap gap-4"><Link to="/inquiry" className="inline-flex min-h-11 items-center gap-2 bg-[var(--site-accent)] px-5 text-sm font-semibold text-zinc-950 transition hover:bg-[var(--site-accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">Inquire <ArrowRight size={17} /></Link>{hasEmail && <a href={`mailto:${content.email}`} className="inline-flex min-h-11 items-center gap-2 border-b border-white/[0.15] text-sm text-zinc-300 hover:text-white"><Mail size={17} /> {content.contactPage.ctaText}</a>}</div>
