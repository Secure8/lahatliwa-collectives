...
Deno.serve(async (request) => {
  ...
  const projectGalleryUploadEnabled = testUploadEnabled;
  if (body.action !== 'verify') return reply({ success: true, configured: env.google.configured, testUploadEnabled, projectGalleryUploadEnabled, connection: safeConnection(connection) }, 200, cors);
  ...
  try {
    ...
    return reply({ success: true, configured: true, testUploadEnabled, projectGalleryUploadEnabled, connection: safeConnection({ ...connection, status: 'connected', root_folder_health: 'healthy', last_verified_at: now, last_error_code: null, last_error_message: null }) }, 200, cors);
  } catch (verifyError) {
